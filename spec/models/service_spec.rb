...
describe Service do
  ...
  it "é inválido sem o nome do serviço" do
    ...
    service.valid?
    expect(service.errors[:service]).to include("não pode ser em branco.")
  end
  ...
  it "o nome do serviço tem que ter até 50 caracteres" do
    ...
    service.valid?
    expect(service.errors[:service]).to include("muito grande. Máximo de 50 caracteres.")
  end

  it "é inválido sem a descrição do serviço" do
    service = build(:service, description: nil)
    service.valid?
    expect(service.errors[:description]).to include("não pode ser em branco.")
  end

  it "é inválido sem o valor o serviço" do
    service = build(:service, value: nil)
    service.valid?
    expect(service.errors[:value]).to include("não pode ser em branco.")
  end

  it "o valor do serviço tem que ter até 4 caracteres" do
    service = build(:service, value: "22222")
    service.valid?
    expect(service.errors[:value]).to include("muito grande. Máximo de 4 caracteres.")
  end

  it "é inválido sem a duração o serviço" do
    service = build(:service, duration: nil)
    service.valid?
    expect(service.errors[:duration]).to include("não pode ser em branco.")
  end

  it "a duração do serviço tem que ter apenas 1 caractere" do
    service = build(:service, duration: "22")
    service.valid?
    expect(service.errors[:duration]).to include("muito grande. Máximo de 1 caracteres.")
  end
